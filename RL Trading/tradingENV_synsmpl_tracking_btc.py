...
class ZiplineEnvSrc(object):
    ...
    def __init__(self, symbol, interval, start, end, freq_cnt=1440, scale=True):
        ...
        symbol_df = self._get_symbol_market(symbol,interval, start, end)
        for base in base_symbols:
            symbol_df = pd.concat([symbol_df, self._get_symbol_market(base,interval, start, end)],axis =1)
        
        self.data = symbol_df
        self.step = 0
        self.orgin_idx = 0

    def _get_symbol_market(self,symbol,interval, start, end):
        log.info('getting data for %s from binance marketing...', symbol)
        ...
        low_price = _df['low'].values
        high_price = _df['high'].values
        ...
        mom = MOM(close, timeperiod=max_time_window)
        ht_dcperiod = HT_DCPERIOD(close)
        ht_dcphase = HT_DCPHASE(close)
        sine, leadsine = HT_SINE(close)
        inphase, quadrature = HT_PHASOR(close)
        adxr = ADXR(high_price, low_price, close, timeperiod=max_time_window)
        apo = APO(close, fastperiod=max_time_window // 2, slowperiod=max_time_window)
        aroon_up, _ = AROON(high_price, low_price, timeperiod=max_time_window)
        cci = CCI(high_price, low_price, close, timeperiod=max_time_window)
        plus_di = PLUS_DI(high_price, low_price, close, timeperiod=max_time_window)
        ppo = PPO(close, fastperiod=max_time_window // 2, slowperiod=max_time_window)
        macd, macd_sig, macd_hist = MACD(close, fastperiod=max_time_window // 2, slowperiod=max_time_window, signalperiod=max_time_window // 2)
        cmo = CMO(close, timeperiod=max_time_window)
        rocp = ROCP(close, timeperiod=max_time_window)
        fastk, fastd = STOCHF(high_price, low_price, close)
        trix = TRIX(close, timeperiod=max_time_window)
        ultosc = ULTOSC(high_price, low_price, close, timeperiod1=max_time_window // 2, timeperiod2=max_time_window, timeperiod3=max_time_window * 2)
        willr = WILLR(high_price, low_price, close, timeperiod=max_time_window)
        natr = NATR(high_price, low_price, close, timeperiod=max_time_window)
    
        df['{}Return'.format(symbol)] = (_df.close - _df.close.shift()) / _df.close.shift()  # today return
        df['{}SMA5'.format(symbol)] = zscore(sma5)
        df['{}SMA15'.format(symbol)] = zscore(sma15)
        df['{}C-SMA5'.format(symbol)] = zscore(close - sma5)
        df['{}SMA5-SMA15'.format(symbol)] = zscore(sma5 - sma15)
        df['{}RSI'.format(symbol)] = zscore(rsi)
        df['{}ATR'.format(symbol)] = zscore(atr)
        df['{}VOL'.format(symbol)] = zscore(_df.volume)
        df['{}CLOSE'.format(symbol)] = zscore(_df.close)
        df['{}MOM'.format(symbol)]= zscore(mom)
        df['{}HT_DCPERIOD'.format(symbol)]= zscore(ht_dcperiod)
        df['{}HT_DCPHASE'.format(symbol)]= zscore(ht_dcphase)
        df['{}sine'.format(symbol)]= zscore(sine)
        df['{}leadsine'.format(symbol)]= zscore(leadsine)
        df['{}inphase'.format(symbol)]= zscore(inphase)
        df['{}quadrature'.format(symbol)]= zscore(quadrature)
        df['{}ADXR'.format(symbol)]= zscore(adxr)
        df['{}APO'.format(symbol)]= zscore(apo)
        df['{}AROON_UP'.format(symbol)]= zscore(aroon_up)
        df['{}CCI'.format(symbol)]= zscore(cci)
        df['{}PLUS_DI'.format(symbol)]= zscore(plus_di)
        df['{}PPO'.format(symbol)]= zscore(ppo)
        df['{}macd'.format(symbol)]= zscore(macd)
        df['{}macd_sig'.format(symbol)]= zscore(macd_sig)
        df['{}macd_hist'.format(symbol)]= zscore(macd_hist)
        df['{}CMO'.format(symbol)]= zscore(cmo)
        df['{}ROCP'.format(symbol)]= zscore(rocp)
        df['{}FASTK'.format(symbol)]= zscore(fastk)
        df['{}FASTD'.format(symbol)]= zscore(fastd)
        df['{}TRIX'.format(symbol)]= zscore(trix)
        df['{}ULTOSC'.format(symbol)]= zscore(ultosc)
        df['{}WILLR'.format(symbol)]= zscore(willr)
        df['{}NATR'.format(symbol)]= zscore(natr)
...
class TradingEnv(object):
    ...
    def __init__(self, symbol, interval, start, end, window, freq_cnt=1440, random = True):
        self.freq_cnt = freq_cnt
        self.src = ZiplineEnvSrc(symbol=symbol, interval=interval, start=start, end=end, freq_cnt=self.freq_cnt, )
        #freq_cnt-window-1, first 20 records for window, and -1 for last 1 record for return only
        self.sim = TradingSim(steps=self.freq_cnt-window-1, trading_cost_bps=1e-4, time_cost_bps=1e-4)  # TODO FIX
    ...
    def _rewards_ls(self, observation, action_ls):
        ...
        for i,act in enumerate(action_ls):
            yret = observation[i+1][-1][0] #return
