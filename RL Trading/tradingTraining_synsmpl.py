...
def execute(symbol, begin, end, interval, train_round,  model_path, freq_cnt):
    ...
    env = tradingENV_synsmpl.TradingEnv(symbol=symbol, interval=interval, start=begin, end=end, freq_cnt=freq_cnt)
    ...
    for episode in range(EPISODES):
        ...
        while not done:            
            ...
            if done:
                ...
                action_ls = agent.get_action(state_ls[:-1],freq_cnt,action_size)
                agent.rewards = env._rewards_ls(state_ls,action_ls)
                agent.states = state_ls[:-1]                
                df = env.sim.to_df()
